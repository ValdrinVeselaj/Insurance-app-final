...
export default function LandinPage() {
    ...
    useEffect(async () => {
        try {
            const { data } = await axios.get(`${process.env.REACT_APP_API_BASEURL}/api/items/siguroShendetin3`, {
    ...
    async function deleteHandler(id) {
        const { data } = await axios.delete(`${process.env.REACT_APP_API_BASEURL}/api/items/siguroShendetin3/${id}`, {
            headers: { 'x-auth-token': accessToken }
        });
    ...
    async function updateHandler(e, id) {
        e.preventDefault();
        const { data } = await axios.post(`${process.env.REACT_APP_API_BASEURL}/api/items/siguroShendetin3/${id}`, editItem, {
            headers: { 'x-auth-token': accessToken }
        });
                    ...
                    list.map(row => {
                        if (row.toEdit) return (
                            <li key={row._id} style={{ 'marginBottom': '20px', 'marginTop': '30px' }}>
                                <form action={`${process.env.REACT_APP_API_BASEURL}/api/items/siguroShendetin3/${row._id}`} onSubmit={(e) => updateHandler(e, row._id)}>
                                    ...
                                    <Input type="text" name='peopleNumber' defaultValue={row.peopleNumber} onChange={(e) => setEditItem({ ...editItem, peopleNumber: e.target.value })} />
                                    ...
                                    <Input type="text" name='country' defaultValue={row.country} onChange={(e) => setEditItem({ ...editItem, country: e.target.value })} />
                                    <Input type="text" name='dateOfBirth' marginBottom={'30px'} defaultValue={row.dateOfBirth} onChange={(e) => setEditItem({ ...editItem, dateOfBirth: e.target.value })} />
                        ...
                        return (
                            <li key={row._id} style={{ 'marginBottom': '20px', 'marginTop': '30px' }}>
                                ...
                                <Input type="text" name='peopleNumber' defaultValue={row.peopleNumber} disabled />
