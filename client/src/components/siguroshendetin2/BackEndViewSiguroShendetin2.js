...
export default function LandinPage() {
    ...
    useEffect(async () => {
        try {
            const { data } = await axios.get(`${process.env.REACT_APP_API_BASEURL}/api/items/siguroShendetin2`, {
    ...
    async function deleteHandler(id) {
        const { data } = await axios.delete(`${process.env.REACT_APP_API_BASEURL}/api/items/siguroShendetin2/${id}`, {
            headers: { 'x-auth-token': accessToken }
        });
                    ...
                    list.map(row => {
                        if (row.toEdit) return (
                            <li key={row._id} style={{ 'marginBottom': '20px', 'marginTop': '30px' }}>
                                <form action={`${process.env.REACT_APP_API_BASEURL}//api/items/siguroShendetin2/${row._id}`} onSubmit={(e) => updateHandler(e, row._id)}>
