...
export default function LandinPage() {
  ...
  useEffect(async () => {
    try {
      const { data } = await axios.get(`${process.env.REACT_APP_API_BASEURL}/api/items/tpl`, {
  ...
  async function deleteHandler(id) {
    const { data } = await axios.delete(`${process.env.REACT_APP_API_BASEURL}/api/items/tpl/${id}`, {
      headers: { 'x-auth-token': accessToken }
    });
  ...
  async function updateHandler(e, id) {
    e.preventDefault();
    const { data } = await axios.post(`${process.env.REACT_APP_API_BASEURL}/api/items/tpl/${id}`, editItem, {
      headers: { 'x-auth-token': accessToken }
    });
          ...
          list.map(row => {
            if (row.toEdit) return (
              <li key={row._id} style={{ 'marginBottom': '20px', 'marginTop': '30px' }}>
                <form action={`${process.env.REACT_APP_API_BASEURL}/api/items/tpl/${row._id}`} onSubmit={(e) => updateHandler(e, row._id)}>
                  ...
                  <Input type="text" name='plateNumber' marginBottom={'30px'} defaultValue={row.plateNumber} onChange={(e) => setEditItem({ ...editItem, plateNumber: e.target.value })} />
            ...
            return (
              <li key={row._id} style={{ 'marginBottom': '20px', 'marginTop': '30px' }}>
                <Input type="text" name='namelastName' defaultValue={row.idNumber} disabled />
                ...
                <Input type="text" name='plateNumber' marginBottom={'30px'} defaultValue={row.plateNumber} disabled />
